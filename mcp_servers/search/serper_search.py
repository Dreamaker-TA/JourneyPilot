...
def _format_search_error(e: Exception, api_name: str = "Serper") -> str:
    """统一的搜索错误格式化函数"""
    if isinstance(e, httpx.HTTPStatusError):
        return f"{api_name} Web Search API HTTP error occurred: {e.response.status_code} - {e.response.text}"
    elif isinstance(e, httpx.RequestError):
        return f"Error communicating with {api_name} Web Search API: {str(e)}"
    else:
...
parser.add_argument(
    ...
    help='传输方式：stdio（默认，供 MCPManager 子进程接入）或 sse（监听 34011 端口）',
)
