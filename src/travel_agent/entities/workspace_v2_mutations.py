...
class UpdateStopScheduleMutation(StrictModel):
    ...
    def require_change(self) -> "UpdateStopScheduleMutation":
        if not (
            {"planned_start", "planned_end", "duration_minutes"} & self.model_fields_set
        ):
...
class UpdateCustomBlockMutation(StrictModel):
    ...
    def require_change(self) -> "UpdateCustomBlockMutation":
        if not (
            {"title", "note", "planned_start", "planned_end", "duration_minutes"}
            & self.model_fields_set
        ):
            raise ValueError(
                "custom block mutation requires at least one changed field"
            )
        return self
...
class UpdateTransportModePreferenceMutation(StrictModel):
    type: Literal["update_transport_mode_preference"] = (
        "update_transport_mode_preference"
    )
...
SelectableEntity = Annotated[
    Union[DiningStop, LodgingStay, VisitStop, TransportLeg], Field(discriminator="type")
]
...
class SelectionInversePatch(StrictModel):
    ...
    previous_slot_status: Literal[
        "researching", "ready", "refreshing", "needs_user_decision"
    ]
...
def _slot(workspace: TripWorkspaceV2, selection_slot_id: str) -> SelectionSlot:
    ...
    raise WorkspaceV2MutationError(
        "selection_slot_not_found", "SelectionSlot does not exist"
    )
...
def _option(slot: SelectionSlot, option_id: str) -> SelectionOption:
    for option in slot.options:
        if option.option_id == option_id:
            return option
    raise WorkspaceV2MutationError(
        "selection_option_not_found", "Option does not belong to SelectionSlot"
    )


def _lineage(
    candidate: DiningCandidate | LodgingCandidate,
    slot: SelectionSlot,
...
def _selection_reason(option: SelectionOption) -> str:
    return "；".join(
        reason.strip() for reason in option.selection_reasons if reason.strip()
    )
...
def _replace_dining(
    ...
    slot: SelectionSlot,
    option: SelectionOption,
    candidate: DiningCandidate,
) -> tuple[DiningStop, DiningStop]:
    current = next(
        (
            item
            for item in workspace.itinerary.dining_stops
            if item.item_id == slot.target_entity_id
        ),
        None,
    )
    if current is None:
        raise WorkspaceV2MutationError(
            "canonical_entity_not_found", "DiningStop target does not exist"
        )
    if current.meal_type not in candidate.meal_types:
        raise WorkspaceV2MutationError(
            "meal_context_mismatch", "Candidate does not serve the slot meal"
        )
...
def _replace_lodging(
    workspace: TripWorkspaceV2,
    slot: SelectionSlot,
    option: SelectionOption,
    candidate: LodgingCandidate,
) -> tuple[LodgingStay, LodgingStay]:
    current = next(
        (
            item
            for item in workspace.itinerary.lodging_stays
            if item.stay_id == slot.target_entity_id
        ),
        None,
    )
    if current is None:
        raise WorkspaceV2MutationError(
            "canonical_entity_not_found", "LodgingStay target does not exist"
        )
    if (
        candidate.check_in_date != current.check_in_date
        or candidate.check_out_date != current.check_out_date
    ):
        raise WorkspaceV2MutationError(
            "stay_context_mismatch", "Candidate does not cover the complete stay"
        )
...
def _replace_visit_stop(
    ...
    current = next(
        (
            item
            for item in workspace.itinerary.visit_stops
            if item.item_id == slot.target_entity_id
        ),
        None,
    )
    if current is None:
        raise WorkspaceV2MutationError(
            "canonical_entity_not_found", "VisitStop target does not exist"
        )
    planned_end = (
...
def _replace_transport_leg(
    ...
    if current is None:
        raise WorkspaceV2MutationError(
            "canonical_entity_not_found", "TransportLeg target does not exist"
        )
...
def _validated_workspace(
    workspace: TripWorkspaceV2, itinerary: StructuredItineraryV2
) -> TripWorkspaceV2:
...
def _itinerary_result(
    workspace: TripWorkspaceV2, itinerary: StructuredItineraryV2, label: str
...
def _timeline_location(
    itinerary: StructuredItineraryV2, entity_id: str
) -> tuple[int, int, TimelineEntryRef]:
    matches = [
    ...
    if len(matches) != 1:
        raise WorkspaceV2MutationError(
            "timeline_item_not_unique", "Timeline item must have exactly one projection"
        )
    return matches[0]
...
def _endpoint_for_ref(
    itinerary: StructuredItineraryV2, ref: TimelineEntryRef
    ...
    entity = next(
        (item for item in items if getattr(item, identity) == ref.entity_id), None
    )
    return (
        None
        if entity is None
        else TransportEndpoint(name=entity.name, place_id=entity.place_id)
    )


def _invalidate_changed_connectors(
    itinerary: StructuredItineraryV2,
) -> StructuredItineraryV2:
    ...
    for day in itinerary.day_plans:
        for index, ref in enumerate(day.timeline):
            ...
            previous = next(
                (
                    _endpoint_for_ref(itinerary, item)
                    for item in reversed(day.timeline[:index])
                    if _endpoint_for_ref(itinerary, item)
                ),
                None,
            )
            following = next(
                (
                    _endpoint_for_ref(itinerary, item)
                    for item in day.timeline[index + 1 :]
                    if _endpoint_for_ref(itinerary, item)
                ),
                None,
            )
            if (
                ...
                or (previous == leg.from_endpoint and following == leg.to_endpoint)
            ):
                continue
            legs[ref.entity_id] = leg.model_copy(
                update={
                    "from_endpoint": previous,
                    "to_endpoint": following,
                    "segments": [],
                    "transfer_count": 0,
...
def _apply_selection(
    workspace: TripWorkspaceV2, mutation: SelectOptionMutation
) -> WorkspaceV2MutationApplication:
    ...
    if slot.status not in {"ready", "needs_user_decision"}:
        raise WorkspaceV2MutationError(
            "selection_slot_not_ready", "SelectionSlot is not selectable"
        )
    candidate = workspace.recommendation_catalog.candidate_index().get(
        option.candidate_id
    )
    ...
    if candidate is None or admission is None or admission.status != "passed":
        raise WorkspaceV2MutationError(
            "candidate_not_admitted", "Candidate did not pass slot admission"
        )
    if getattr(candidate, "availability_status", None) == "unavailable":
        raise WorkspaceV2MutationError(
            "candidate_unavailable", "Unavailable candidate cannot be selected"
        )
    ...
    if candidate_option_availability(candidate) != option.availability_status:
        raise WorkspaceV2MutationError(
            "availability_mismatch", "Option availability differs from candidate"
        )
    itinerary = workspace.itinerary
    if slot.slot_type == "dining":
        if (
            not isinstance(candidate, DiningCandidate)
            or option.candidate_entity_ref.entity_type != EntityType.DINING_STOP
        ):
            raise WorkspaceV2MutationError(
                "candidate_type_mismatch", "Dining slot requires DiningCandidate"
            )
        previous, updated = _replace_dining(workspace, slot, option, candidate)
        itinerary = itinerary.model_copy(
            update={
                "dining_stops": [
                    updated if item.item_id == previous.item_id else item
        ...
        if (
            not isinstance(candidate, VisitCandidate)
            or option.candidate_entity_ref.entity_type != EntityType.VISIT_STOP
        ):
            raise WorkspaceV2MutationError(
                "candidate_type_mismatch", "Visit slot requires VisitCandidate"
            )
        ...
        if (
            ...
            or option.candidate_entity_ref.entity_type != EntityType.TRANSPORT_LEG
        ):
            raise WorkspaceV2MutationError(
                "candidate_type_mismatch", "Transport slot requires TransportCandidate"
            )
        ...
        itinerary = itinerary.model_copy(
            update={
                "transport_legs": [
                    updated
                    if item.transport_leg_id == previous.transport_leg_id
                    else item
        ...
        if (
            not isinstance(candidate, LodgingCandidate)
            or option.candidate_entity_ref.entity_type != EntityType.LODGING_STAY
        ):
            raise WorkspaceV2MutationError(
                "candidate_type_mismatch", "Lodging slot requires LodgingCandidate"
            )
        previous, updated = _replace_lodging(workspace, slot, option, candidate)
        itinerary = itinerary.model_copy(
            update={
                "lodging_stays": [
                    updated if item.stay_id == previous.stay_id else item
                    for item in itinerary.lodging_stays
                ],
            }
        )
    itinerary = _recompute_derived_itinerary_fields(itinerary)
    next_slot = slot.model_copy(
        update={"selected_option_id": option.option_id, "status": "ready"}
    )
    next_workspace = TripWorkspaceV2.model_validate(
        workspace.model_copy(
            update={
                ...
                "selection_slots": [
                    next_slot
                    if item.selection_slot_id == slot.selection_slot_id
                    else item
    ...
    return WorkspaceV2MutationApplication(
        workspace=next_workspace,
        changed=True,
        label=_selection_label(updated),
        inverse=SelectionInversePatch(
            selection_slot_id=slot.selection_slot_id,
            applied_option_id=option.option_id,
            applied_workspace_revision=next_workspace.workspace_revision,
            previous_selected_option_id=slot.selected_option_id,
            previous_slot_status=slot.status,
...
def _proposal(
    ...
    if weather is None:
        raise WorkspaceV2MutationError(
            "weather_snapshot_required",
            "Weather adjustment requires the current Weather snapshot",
        )
    proposal = next(
        (
            item
            for item in weather.adjustment_proposals
    ...
    if any(
        item.proposal_id == proposal_id for item in workspace.weather_proposal_decisions
    ):
        raise WorkspaceV2MutationError(
            "weather_proposal_resolved",
            "Weather adjustment proposal was already handled",
        )
    return proposal
...
def _replace_visit_candidate(
    ...
    if (
        ...
        raise WorkspaceV2MutationError(
            ...
            "Weather replacement candidate did not pass admission",
        )
    planned_end = (
        current.planned_start
        + timedelta(minutes=candidate.recommended_duration_minutes)
...
def _operation_mutation(workspace: TripWorkspaceV2, operation):
    ...
    if isinstance(operation, WeatherBufferOperation):
        day = next(
            (
                item
                for item in workspace.itinerary.day_plans
                if item.day_id == operation.day_id
            ),
            None,
        )
        if day is None:
            raise WorkspaceV2MutationError(
                "weather_proposal_stale", "Weather buffer day changed"
            )
        target_index = next(
            (
                index
...
def _apply_weather_adjustment(
    ...
    for operation in proposal.operations:
        if isinstance(operation, WeatherVisitReplacementOperation):
            ...
            if nested is None:
                raise WorkspaceV2MutationError(
                    "weather_operation_unsupported",
                    "Weather proposal operation is unsupported",
                )
    ...
    if not changed:
        raise WorkspaceV2MutationError(
            "weather_proposal_no_change",
            "Weather proposal no longer changes the itinerary",
        )
    decisions = [
...
def apply_workspace_v2_mutation(
    ...
    if isinstance(mutation, MoveTimelineItemMutation):
        ...
        if ref.entity_type not in {
            ...
            raise WorkspaceV2MutationError(
                "item_not_movable", "Only Visit, Dining, and Custom items can be moved"
            )
        target_day = next(
            (
                index
                for index, day in enumerate(itinerary.day_plans)
                if day.day_id == mutation.to_day_id
            ),
            None,
        )
        ...
        if mutation.before_entry_id is None:
            ...
            before = next(
                (
                    index
                    for index, item in enumerate(target_timeline)
                    if item.entry_id == mutation.before_entry_id
                ),
                None,
            )
            if before is None:
                raise WorkspaceV2MutationError(
                    "before_entry_not_found",
                    "Insertion anchor does not exist in target day",
                )
            target_timeline.insert(before, moved)
        updates = {"day_plans": day_plans}
        key = {
            EntityType.VISIT_STOP: "visit_stops",
            EntityType.DINING_STOP: "dining_stops",
            EntityType.CUSTOM_BLOCK: "custom_blocks",
        }[ref.entity_type]
        ...
        next_itinerary = _invalidate_changed_connectors(
            itinerary.model_copy(update=updates)
        )
    ...
    if isinstance(mutation, UpdateStopScheduleMutation):
        for key in ("visit_stops", "dining_stops", "custom_blocks"):
            ...
            current = next(
                (item for item in items if item.item_id == mutation.item_id), None
            )
            if current is None:
                continue
            values = {
                name: getattr(mutation, name)
                for name in ("planned_start", "planned_end", "duration_minutes")
                if name in mutation.model_fields_set
            }
            updated = current.model_copy(update=values)
            if (
                updated.planned_start
                and updated.planned_end
                and updated.planned_end <= updated.planned_start
            ):
                raise WorkspaceV2MutationError(
                    "invalid_schedule", "planned_end must be after planned_start"
                )
            return _itinerary_result(
                workspace,
                itinerary.model_copy(
        ...
        raise WorkspaceV2MutationError(
            "scheduled_item_not_found", "Scheduled item does not exist"
        )
    if isinstance(mutation, CreateCustomBlockMutation):
        if mutation.block.day_id not in {day.day_id for day in itinerary.day_plans}:
            raise WorkspaceV2MutationError(
                "day_not_found", "Custom block day does not exist"
            )
        if any(
            item.item_id == mutation.block.item_id for item in itinerary.custom_blocks
        ):
            raise WorkspaceV2MutationError(
                "item_id_conflict", "Custom block id already exists"
            )
        day_plans = [day.model_copy(deep=True) for day in itinerary.day_plans]
        day = next(item for item in day_plans if item.day_id == mutation.block.day_id)
        ref = TimelineEntryRef(
            entry_id=f"entry_{mutation.block.item_id}",
            entity_type=EntityType.CUSTOM_BLOCK,
            entity_id=mutation.block.item_id,
        )
        if mutation.before_entry_id is None:
            ...
            before = next(
                (
                    i
                    for i, item in enumerate(day.timeline)
                    if item.entry_id == mutation.before_entry_id
                ),
                None,
            )
            if before is None:
                raise WorkspaceV2MutationError(
                    "before_entry_not_found", "Insertion anchor does not exist"
                )
        ...
        return _itinerary_result(
            ...
            itinerary.model_copy(
                update={
                    "day_plans": day_plans,
                    "custom_blocks": [*itinerary.custom_blocks, mutation.block],
                }
            ),
            f"添加{mutation.block.title}",
        )
    if isinstance(mutation, UpdateCustomBlockMutation):
        current = next(
            (
                ...
                if item.item_id == mutation.item_id
            ),
            None,
        )
        if current is None:
            raise WorkspaceV2MutationError(
                "custom_block_not_found", "Custom block does not exist"
            )
        values = {
            name: getattr(mutation, name)
            for name in (
    ...
    if isinstance(mutation, DeleteCustomBlockMutation):
        if not any(
            item.item_id == mutation.item_id for item in itinerary.custom_blocks
        ):
            raise WorkspaceV2MutationError(
                "custom_block_not_found", "Custom block does not exist"
            )
            ...
            day.model_copy(
                update={
                    "timeline": [
                        ...
                        if not (
                            ref.entity_type == EntityType.CUSTOM_BLOCK
                            and ref.entity_id == mutation.item_id
                        )
                    ]
                }
            )
            ...
            day.model_copy(
                update={
                    "timeline": [
                        ...
                        for ref in day.timeline
                        if not (
                            ref.entity_type == EntityType.TRANSPORT_LEG
                            and ref.entity_id == mutation.transport_leg_id
                        )
                    ]
                }
            )
    ...
    if isinstance(mutation, SetTransportModeMutation):
        ...
        if current.transport_class == "long_distance":
            raise WorkspaceV2MutationError(
                "long_distance_requires_option",
                "Long-distance transport requires explicit option replacement",
            )
        if mutation.selected_mode in current.mode_preference.excluded_modes:
            raise WorkspaceV2MutationError(
                "transport_mode_excluded",
                "Selected mode is excluded by user preference",
            )
        preference = current.mode_preference.model_copy(
            update={
                "locked_mode": mutation.selected_mode if mutation.lock_mode else None
            }
        )
        if preference != current.mode_preference:
            _check_preference_leaves_a_way_back(workspace, current, preference)
        if current.selected_mode == mutation.selected_mode:
            if preference == current.mode_preference:
                return WorkspaceV2MutationApplication(
                    workspace=workspace, changed=False
                )
            updated = current.model_copy(update={"mode_preference": preference})
            return _itinerary_result(
                workspace,
        ...
        updated = current.model_copy(
            update={
                "selected_mode": mutation.selected_mode,
    ...
    raise WorkspaceV2MutationError(
        "unsupported_operation", "Unsupported workspace v2 mutation"
    )
...
def apply_workspace_v2_inverse(
    ...
    *,
    ...
    if workspace.workspace_revision != inverse.applied_workspace_revision and not (
        ...
        raise WorkspaceV2MutationError(
            "undo_head_mismatch", "Workspace changed since this mutation"
        )
...
def apply_selection_inverse(
    ...
    *,
    ...
    if slot.selected_option_id != inverse.applied_option_id:
        raise WorkspaceV2MutationError(
            "undo_head_mismatch", "Selection changed since this mutation"
        )
    if inverse.previous_selected_option_id is not None:
        ...
        if previous_option.candidate_id != inverse.previous_entity.lineage.candidate_id:
            raise WorkspaceV2MutationError(
                "inverse_lineage_mismatch",
                "Inverse entity does not match previous option",
            )
    itinerary = workspace.itinerary
    if isinstance(inverse.previous_entity, DiningStop):
        itinerary = itinerary.model_copy(
            update={
                "dining_stops": [
                    inverse.previous_entity
                    if item.item_id == inverse.previous_entity.item_id
                    else item
                    for item in itinerary.dining_stops
                ],
            }
        ...
        itinerary = itinerary.model_copy(
            update={
                "transport_legs": [
                    inverse.previous_entity
                    if item.transport_leg_id == inverse.previous_entity.transport_leg_id
                    else item
                    for item in itinerary.transport_legs
                ],
            }
        ...
        itinerary = itinerary.model_copy(
            update={
                "lodging_stays": [
                    inverse.previous_entity
                    if item.stay_id == inverse.previous_entity.stay_id
                    else item
                    for item in itinerary.lodging_stays
                ],
            }
    ...
    next_slot = slot.model_copy(
        update={
            "selected_option_id": inverse.previous_selected_option_id,
            "status": inverse.previous_slot_status,
        }
    )
    return TripWorkspaceV2.model_validate(
        workspace.model_copy(
